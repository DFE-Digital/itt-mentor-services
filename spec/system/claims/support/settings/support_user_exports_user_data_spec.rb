...
RSpec.describe "Support user exports user data", service: :claims, type: :system do
  scenario do
    ...
    then_i_see_an_error_summary("Select which claim windows to include")
    ...
    when_i_click_continue_without_selecting_an_option
    then_i_see_an_error_summary("Select a claim window")
    ...
    when_i_click_continue_without_selecting_an_option
    then_i_see_an_error_summary("Select which users to include")
  ...
  def then_i_see_the_activity_level_step
    ...
    expect(page).to have_field("Only active users", type: :radio)
  end
  ...
  def then_i_receive_a_csv_download
    ...
    expect(csv.headers).to include("ID", "Name", "Email")
  end
  ...
  def then_i_see_an_error_summary(message)
    expect(page).to have_css(".govuk-error-summary")
    expect(page).to have_css(".govuk-error-summary__list li", text: message)
  end
end
