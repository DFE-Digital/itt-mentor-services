...
RSpec.describe "Support user views rejected by payer activity spec", service: :claims, type: :system do
  scenario do
    given_rejected_by_payer_activity_exists
    and_i_am_signed_in
  ...
  def given_rejected_by_payer_activity_exists
    ...
    @activity_log = create(:claim_activity, :rejected_by_payer, user: @colin, record: @best_practice_network_claim)
  end
  ...
  def then_i_see_the_activity_log_page
    ...
    expect(page).to have_element("h3", class: "app-timeline__title", text: "Payer rejected payment for claim 12345678")
  ...
  def then_i_see_the_activity_details
    expect(page).to have_title("Payer rejected payment for claim 12345678 - Claim funding for mentor training - GOV.UK")
    expect(primary_navigation).to have_current_item("Claims")
    expect(page).to have_h1("Payer rejected payment for claim 12345678")
