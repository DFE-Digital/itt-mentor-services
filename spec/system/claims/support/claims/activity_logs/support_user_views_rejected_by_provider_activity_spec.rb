...
RSpec.describe "Support user views rejected by provider activity spec", service: :claims, type: :system do
  ...
  def then_i_see_the_activity_log_page
    ...
    expect(page).to have_element("h3", class: "app-timeline__title", text: "Provider #{@best_practice_network_claim.provider_name} rejected audit for claim 12345678")
  ...
  def then_i_see_the_activity_details
    expect(page).to have_title("Provider #{@best_practice_network_claim.provider_name} rejected audit for claim 12345678 - Claim funding for mentor training - GOV.UK")
    expect(primary_navigation).to have_current_item("Claims")
    expect(page).to have_h1("Provider #{@best_practice_network_claim.provider_name} rejected audit for claim 12345678")
