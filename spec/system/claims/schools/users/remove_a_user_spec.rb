...
RSpec.describe "Remove a user", service: :claims, type: :system do
  ...
  scenario "Claims user cannot see remove user link for their own user" do
    given_a_school_exists_with_claims_users
    and_i_am_signed_in
  ...
  scenario "Claims user prevented from accessing the remove user route for their own user" do
    given_a_school_exists_with_claims_users
    and_i_am_signed_in
  ...
  def then_i_see_the_removal_confirmation_page
    ...
    expect(page).to have_caption("Barry Garlow")
