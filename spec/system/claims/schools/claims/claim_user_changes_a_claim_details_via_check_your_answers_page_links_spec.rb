...
RSpec.describe "Claims user changes a claim details via check your answers page links", :js, service: :claims, type: :system do
  scenario do
    ...
    then_i_see_the_select_provider_step
    and_the_provider_is_prefilled_with_best_practice_network
    ...
    and_i_click_on_continue
    then_i_see_the_select_mentors_step_with_amended_provider_and_both_mentors_preselected
  ...
  def when_i_enter_a_provider_named_niot
    fill_in "Enter the accredited provider", with: "NIoT: National Institute of Teaching, founded by the School-Led Development Trust"
  end

  def then_i_see_a_dropdown_item_for_niot
    expect(page).to have_css(".autocomplete__option", text: "NIoT: National Institute of Teaching, founded by the School-Led Development Trust", wait: 10)
  end

  def when_i_click_the_dropdown_item_for_niot
    page.find(".autocomplete__option", text: "NIoT: National Institute of Teaching, founded by the School-Led Development Trust").click
  end
  ...
  def then_i_see_the_select_mentors_step_with_amended_provider_and_both_mentors_preselected
    expect(page).to have_title("Select mentors that trained with NIoT: National Institute of Teaching, founded by the School-Led Development Trust - Claim details - Claim funding for mentor training - GOV.UK")
    expect(primary_navigation).to have_current_item("Claims")
    expect(page).to have_hint("Select all teachers that completed training to be initial teacher training (ITT) mentors.")
  ...
  def then_i_see_the_select_training_hours_step_for_barry_garlow_with_persisted_details
    expect(page).to have_title("How many hours of training did Barry Garlow complete? - Claim details - NIoT: National Institute of Teaching, founded by the School-Led Development Trust - Claim funding for mentor training - GOV.UK")
    expect(primary_navigation).to have_current_item("Claims")
    expect(page).to have_element(:h1, class: "govuk-fieldset__heading", text: "How many hours of training did Barry Garlow complete?")
    expect(page).to have_hint("Barry Garlow (TRN 8888888) is eligible for 20 hours of training.")
    expect(page).to have_element(:label, class: "govuk-label govuk-radios__label", text: "20 hours")
  ...
  def then_i_see_the_select_training_hours_step_for_james_jameson_with_persisted_details
    expect(page).to have_title("How many hours of training did James Jameson complete? - Claim details - NIoT: National Institute of Teaching, founded by the School-Led Development Trust - Claim funding for mentor training - GOV.UK")
    expect(primary_navigation).to have_current_item("Claims")
  ...
  def and_i_enter_a_negative_value_for_training_hours
  ...
  def then_i_see_the_claim_submitted_page
    ...
    expect(page).to have_text("NIoT: National Institute of Teaching, founded by the School-Led Development Trust will contact you if your claim undergoes a check.")
  end
  ...
  def and_the_provider_is_prefilled_with_best_practice_network
    expect(page.find("#claims-add-claim-wizard-provider-step-id-field").value).to eq(
      "Best Practice Network",
    )
  end

  def then_i_see_the_check_the_check_answers_page_with_updated_details
    expect(page).to have_title("Check your answers before submitting your claim - Claim details - Claim funding for mentor training - GOV.UK")
    expect(primary_navigation).to have_current_item("Claims")
    expect(page).to have_h1("Check your answers before submitting your claim")
    expect(page).to have_h2("Details")
    expect(page).to have_summary_list_row("School", "Shelbyville Elementary")
    expect(page).to have_summary_list_row("Academic year", @claim_window.academic_year.name.to_s)
    expect(page).to have_summary_list_row("Provider", "NIoT: National Institute of Teaching, founded by the School-Led Development Trust")
    row = find("dt.govuk-summary-list__key", text: "Mentors").ancestor(".govuk-summary-list__row")
    within(row) do
      expect(page).to have_selector("ul.govuk-list li", count: 2)
      expect(page).to have_text("Barry Garlow")
