...
RSpec.describe "ESFA user downloads a clawback CSV with expired token", service: :claims, type: :system do
  scenario do
  ...
  def and_the_token_has_expired
    @token = Rails.application.message_verifier(:clawback).generate(@clawback.id, expires_at: 1.day.ago)
  end
