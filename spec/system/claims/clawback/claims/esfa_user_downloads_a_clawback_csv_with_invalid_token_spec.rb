...
RSpec.describe "ESFA user downloads a clawback CSV with invalid token", service: :claims, type: :system do
  scenario do
  ...
  def and_the_token_is_invalid
    @token = Rails.application.message_verifier(:bobs_burgers).generate(@clawback.id, expires_in: 7.days)
  end
