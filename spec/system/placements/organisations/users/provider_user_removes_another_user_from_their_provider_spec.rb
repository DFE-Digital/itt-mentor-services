...
RSpec.describe "Provider user removes another user from their provider", service: :placements, type: :system do
  ...
  scenario do
    ...
    and_a_user_is_onboarded_to_the_provider
    and_i_am_signed_in
  ...
  def and_a_user_is_onboarded_to_the_provider
    @user = create(
      :placements_user,
      first_name: "Joe",
      last_name: "Bloggs",
  ...
  def then_i_see_the_are_you_sure_page
    ...
    expect(page).to have_paragraph(
      "Joe Bloggs will no longer be able to access placement information.",
    )
    expect(page).to have_warning_text(
      "The user will be sent an email to tell them you deleted them from The London Provider.",
    )
    expect(page).to have_button("Delete user", class: "govuk-button--warning")
    expect(page).to have_link("Cancel", href: placements_provider_user_path(@provider, @user))
  end
