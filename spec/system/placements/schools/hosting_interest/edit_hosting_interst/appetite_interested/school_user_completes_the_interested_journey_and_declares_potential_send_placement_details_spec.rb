...
RSpec.describe "School user completes the interested journey and declares potential send placement details",
               service: :placements,
               type: :system do
  scenario do
  ...
  def and_the_schools_potential_placement_details_have_been_updated
    ...
    expect(potential_placement_details["key_stage"]).to eq({ "key_stages" => [@key_stage_2.id, @key_stage_5.id] })
