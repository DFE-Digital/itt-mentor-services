...
RSpec.describe "School user edits their hosting interest and bulk adds send placements",
               service: :placements,
               type: :system do
  scenario do
  ...
  def and_i_see_send_placements_have_been_created
    expect(page).to have_summary_list_row("Number of SEND placements", "2")
    expect(page).to have_summary_list_row("Key stages", "Key stage 2 Key stage 5")
  end
