...
RSpec.describe "School user completes the interested journey without declaring specific year groups or subjects",
  ...
  def and_the_schools_potential_placement_details_have_been_updated
    ...
    expect(
      potential_placement_details["secondary_subject_selection"],
    ).to eq(
  ...
  def and_i_see_the_potential_primary_placement_details_are_not_known
    expect(page).to have_h2("Potential primary placements", class: "govuk-heading-m")
    within("#primary_placements") do
      expect(page).to have_summary_list_row("I don’t know", "Not entered")
    end
  end

  def and_i_see_the_potential_secondary_placement_details_are_not_known
