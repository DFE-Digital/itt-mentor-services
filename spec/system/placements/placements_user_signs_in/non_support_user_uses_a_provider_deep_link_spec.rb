...
RSpec.describe "Non-support user uses a provider deep link", service: :placements, type: :system do
  scenario do
    given_my_provider_exists
    and_i_have_an_account
    when_i_visit_the_provider_users_path
    then_i_am_redirected_to_the_sign_in_page

    when_i_click_on_sign_in_using_dfe_sign_in
    then_i_am_redirected_to_the_provider_users_page
  end
  ...
  def when_i_visit_the_provider_users_path
  ...
  def then_i_am_redirected_to_the_provider_users_page
    expect(page).to have_title("Users - Manage school placements - GOV.UK")
    expect(page).to have_h1("Users")
    expect(page).to have_link("Add user", href: "/providers/#{@provider.id}/users/new", class: "govuk-button")
  end
end
