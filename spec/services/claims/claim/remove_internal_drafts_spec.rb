...
describe Claims::Claim::RemoveInternalDrafts do
  ...
  describe "#call" do
    it "removes internal draft claims that haven't changed in at least 24 hours" do
      expired_claim = create(:claim, status: :internal_draft, updated_at: 25.hours.ago)
      expect { service }.to change(Claims::Claim, :count).from(4).to(3)
      expect(Claims::Claim.all).to contain_exactly(draft_claim, active_internal_draft_claim, submitted_claim)
      expect(Claims::Claim.all).not_to include(expired_claim)
    end
  end
end
