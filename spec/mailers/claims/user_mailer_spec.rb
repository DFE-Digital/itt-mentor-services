...
RSpec.describe Claims::UserMailer, type: :mailer do
  ...
  describe "#claim_payment_in_progress_notification" do
    subject(:payment_in_progress_email) { described_class.claim_payment_in_progress_notification(user, claim) }
    ...
    it "sends the payment in progress email" do
      ...
      expect(payment_in_progress_email.subject).to eq("Your mentor training claim payment is being processed")
      expect(payment_in_progress_email.body.to_s.squish).to eq(<<~EMAIL.squish)
        Dear Joe,
        ...
        We will send another email when your claim has been paid.

        # Contact us

        If you need help, contact the team at [ittmentor.funding@education.gov.uk](mailto:ittmentor.funding@education.gov.uk).

        [Learn more about funding for mentor training on GOV.UK](https://www.gov.uk/government/publications/initial-teacher-training-reform-funding-guidance)
  ...
  describe "#claim_paid_notification" do
    subject(:paid_email) { described_class.claim_paid_notification(user, claim) }

    let(:user) { create(:claims_user, first_name: "Joe") }
    let(:school) { create(:claims_school, name: "Shelbyville Elementary", region: regions(:inner_london)) }
    let(:claim) do
      create(:claim, :paid, reference: "123", school:, paid_to_la: true, date_paid: Date.new(2026, 9, 21))
    end

    it "sends the claim paid email" do
      expect(paid_email.to).to contain_exactly(user.email)
      expect(paid_email.subject).to eq("Your mentor training claim has been paid")
      expect(paid_email.body.to_s.squish).to eq(<<~EMAIL.squish)
        Dear Joe,
        ...
        * Claim amount: #{claim.amount.format(symbol: true, decimal_mark: ".", no_cents: false)}
        ...
        In line with the conditions of grant, schools should keep evidence of the expenditure they claim for the general mentor role for assurance purposes.
