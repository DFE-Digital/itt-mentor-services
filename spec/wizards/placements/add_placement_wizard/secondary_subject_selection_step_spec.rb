...
RSpec.describe Placements::AddPlacementWizard::SecondarySubjectSelectionStep, type: :model do
  ...
  describe "#subject" do
    ...
    context "when subject_id is set" do
      let(:attributes) { { subject_id: secondary_subject_ids.first } }

      it { is_expected.to eq(secondary_subjects.first) }
    end
