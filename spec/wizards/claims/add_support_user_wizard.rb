...
RSpec.describe Claims::AddSupportUserWizard do
  ...
  describe "#create_support_user" do
    ...
    context "when the support user does not already exist" do
      it "creates a new support user" do
        expect { create_support_user }.to change(
          Claims::SupportUser, :count
        ).by(1)

        support_user = Claims::SupportUser.find_by(first_name:, last_name:, email:)
    ...
    context "when the support user has been discarded" do
      let(:support_user) { create(:claims_support_user, first_name:, last_name:, email:) }
